 ...
 */
SerialCommand::SerialCommand(const Entry *commandList, DefaultHandler defaultHandler, char terminator, const char *delimiters)
 ...
 */
void SerialCommand::setDefaultHandler(DefaultHandler function) {
  ...
  while (Serial.available() > 0) {
    ...
    if (inChar == term) {     // Check for the terminator meaning end of command
      ...
      if (command != NULL) {
        ...
        for (int i = 0; commandList[i].command != NULL; i++) {
          #ifdef SERIALCOMMAND_DEBUG
      ...
      if (bufPos < COMMAND_BUFFER_LENGTH) {
        buffer[bufPos++] = inChar;  // Put character into buffer
        buffer[bufPos] = '\0';      // Null terminate
      } else {
        #ifdef SERIALCOMMAND_DEBUG
          Serial.println("Line buffer is full - increase COMMAND_BUFFER_LENGTH");
        #endif
      }
    }
  }
