...
class SocketsThread (threading.Thread):
  ...
  def run(self):
    ...
    while not self.shouldExit:
      ...
      if r: print('select readers:', len(r))
      if w: print('select writers:', len(w))
...
class LightArms:
  ...
  def __init__(self):
    # 
    self.arms = [
      NetworkArm(addr='localhost', port=10001),#71),
      NetworkArm(addr='localhost', port=10002),#72),
      NetworkArm(addr='localhost', port=10003),#73),
      NetworkArm(addr='localhost', port=10004),#74),
    ]
